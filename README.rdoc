= RDoc-custom-template

RDoc-custom-template is an RDoc generator built with simplicity, beauty
and ease of browsing in mind.

== Installation

    gem install rdoc-custom-template

== Usage

    rdoc -o doc -f custom lib

An alternative is to set the `RDOCOPT` environment variable:

    RDOCOPT="-f custom"

This will make RDoc always use rdoc-custom-templates unless it is explicitly overridden.

== Integrating with RubyGems

If you put the following line in your .gemrc, this will make
RubyGems use RDoc-custom-template for all rdoc generation:

    rdoc: -f custom

After doing this, if you wish to have documentation for all your installed gems
to be formatted in RDocs-custom-template:
...
RDoc-custom-template as the RDoc generator:
...
RDoc-custom-template makes a small modification to RDoc to generate more
